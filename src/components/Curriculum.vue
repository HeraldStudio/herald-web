<template lang="pug">
  ...
  .widget.curriculum(v-if='curriculum' :class='{ stale: curriculum && curriculum.isStale }')
    ...
    .week-picker(v-if='type === "week" || type === "outline"')
      ...
      .cur(v-if='type === "week"' title='点击回到本周' @click='()=>{this.displayWeek=Math.min(this.maxWeek, Math.max(1, this.currentWeek))}') 第 {{ displayWeek }} 周
    ...
    div.curriculum-container(v-if='type === "week"')
      ...
      .curriculum-list(v-if='fixedClasses.length || !floatClasses.length' :class='{ empty: !fixedClasses.length }')
        ...
        .normal-block(v-for='item in fixedNormalClasses' v-if='item.dayOfWeek' :class='{ selected: item === selected }' :style='getBlockStyle(item)' @click='select(item)')
          .name {{ item.courseName }}
          .teacher {{ item.teacherName }}
          .place {{ item.location }}
        .custom-block(v-for='item in fixedCustomClasses' v-if='item.dayOfWeek' :class='{ selected: item === selected }' :style='getBlockStyle(item)' @click='select(item)')
          .name {{ item.courseName }}
          .teacher {{ item.teacherName }}
          .place {{ item.location }}
...
export default {
  ...
  watch: {
    // 改变显示学期时，触发课表拉取
    async displayTerm() {
      this.curriculum = await api.get("/api/curriculum", {
        term: this.displayTerm
      });
      this.selected = null;
    },
    ...
    curriculum() {
      if (this.curriculum.term.startDate) {
        ...
        this.currentWeek = Math.ceil(
          (now.getTime() - this.curriculum.term.startDate) /
            (1000 * 60 * 60 * 24 * 7)
        );
        ...
        this.displayWeek = Math.min(
          this.maxWeek,
          Math.max(1, this.currentWeek)
        );
      } else {
  ...
  methods: {
    ...
    prevTerm() {
      ...
      this.displayTerm =
        term[(term.indexOf(this.displayTerm) + term.length - 1) % term.length];
    },
    // 显示下一学期，带边界检查
    nextTerm() {
      let term = this.term.list.map(k => k.name);
      this.displayTerm =
        term[(term.indexOf(this.displayTerm) + 1) % term.length];
    },
    // 获取当前显示周中某日的日期
    getDate(dayOfWeek) {
      ...
      let t =
        startDate +
        ((this.displayWeek - 1) * 7 + (dayOfWeek - 1)) * (1000 * 60 * 60 * 24);
      let dt = new Date(t);
      let today = new Date();
    ...
    getBlockStyle(course) {
      if (course._id) {
        ...
        return {
          ...
          height:
            ((course.endPeriod - course.beginPeriod + 1) / 13) * 99.8 + "%",
        };
      }
      //定义普通课程样式
      return {
  ...
  computed: {
    // 概览视图用，所有课程按名称和教师去重
    allClasses() {
      let allClasses = this.curriculum.curriculum
        .map(k => ({
    ...
    // 条件：已开课、未结课、单双周匹配
    displayClasses() {
      return this.curriculum.curriculum.filter(
        k =>
    ...
    upcomingClasses() {
      ...
      return this.curriculum.curriculum
        ...
        .filter(
          k => k.endTime > now && k.startTime < now + 3 * 24 * 60 * 60 * 1000
        );
    },
    // 取固定课程（有具体时间的课程）
    fixedClasses() {
    ...
    //取固定课程中的普通课程
        fixedNormalClasses() {
      return this.fixedClasses.filter(k => !k._id);
    },
        //取固定课程中的自定义课程
        fixedCustomClasses() {
      return this.fixedClasses.filter(k => k._id);
    ...
    .curriculum-list {
      ...
      .custom-block {
        ...
        &.selected {
          ...
          box-shadow: 5px 5px 15px -8px var(--color-purple);
          border-radius: 2px;
        }
